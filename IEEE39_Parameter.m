...
%% obtain parameters from case39
slack_bus = 31;                              %slack_bus
define_constants;

gen_node = mpc.gen(:,BUS_I);
line     = mpc.branch;
node     = mpc.bus;
gen_num  = length(gen_node);
line_num = length(line);
node_num = length(node);

% generating gen_map with gen_num rows and line_num cols
gen_map = zeros(gen_num,line_num);
for i = 1:gen_num
    gen_map(i,gen_node(i)) = 1;
end

% generating line_map with line_num rows and node_num cols
line_map = zeros(line_num,node_num);
for i = 1:line_num
    line_map(i,line(i,F_BUS)) = 1;
    line_map(i,line(i,T_BUS)) = -1;
end

l_max = line(:,RATE_A);
admittance = 1./line(:,BR_X)*5; 


%% linearization for quadratic cost, 3 segments
M =3;                              
...
% first point
Pg_min =  mpc.gen(:,PMIN);              % set as 40% of the maximal power output
Pgs_max = (Pg_max - Pg_min)/3;
Pg_k2 =  (Pg_max - Pg_min)/3 + Pg_min;
Pg_k3 =  2*(Pg_max - Pg_min)/3 + Pg_min;

% fixed cost
fixed_cost_min = a .* Pg_min.^2 + b.*Pg_min + c;
fixed_cost_k2  = a .* Pg_k2.^2 + b.*Pg_k2 + c;
fixed_cost_k3  = a .* Pg_k3.^2 + b.*Pg_k3 + c; 
fixed_cost_max = a .* Pg_max.^2 + b.*Pg_max + c;

% cost slope
k = zeros(gen_num,3);
k(:,1) = (fixed_cost_k2 - fixed_cost_min)./(Pg_k2 - Pg_min);
k(:,2) = (fixed_cost_k3 - fixed_cost_k2)./(Pg_k3 - Pg_k2);
k(:,3) = (fixed_cost_max - fixed_cost_k3)./(Pg_max - Pg_k3);

%% for load
demand    = min(mpc.bus(:,PD),250);
sum_load  = sum(demand);

%% cost information
C_g = k(:,3);                      % the PFR cost of generators
C_s = [min(C_g); max(C_g)];        % the PFR cost of DSR
...
R_max   = [PFR_max, DSR_max]; 

min_total_reserve = 240;                              % 80% of the power loss: 0.8 * 300
min_gen_PFR = 210;                                    % 70% PFR should be provided by generators

FD_max = 20;                                          % maximum frequency deviation criterion
rho = 1.0;                                            % security preference of the ISO, must be >= 0.25, otherwise, possible to be infeasible   

% tunning parameter to enlarge the sample variance and largest sample residue
gamma1 = 2;
gamma2 = 2;

% load the linear regression parameters
load('Coefficient.mat','Coefficient');
slope = Coefficient(2:13);
intercept = Coefficient(1);

load('max_regression_residue.mat','max_regression_residue');
xi_max = gamma1 * max_regression_residue;
xi_min = -gamma1 * max_regression_residue;

load('residue_variance.mat','residue_variance');
sigma = gamma2 * residue_variance;       